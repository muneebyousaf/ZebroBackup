	...
	while (1)
	{
		...
		if(!USE_COLOR || toggled != frame_counter - 1)
		{
			...
			if(toggle_image_type == STEREO_PIXMUX)
			{
				...
				stereo_vision_Kirk(current_image_buffer, disparity_image_buffer_8bit, image_width, image_height, disparity_range, thr1, thr2, min_y, max_y);
				...
				disparities_high = evaluate_disparities(disparity_image_buffer_8bit, image_width, image_height, disparity_threshold, disparities_high);
			...
			else
			{
				...
				n_red_pixels = filter_red_color(current_image_buffer, filtered_image, image_width, image_height, min_U, max_U, min_V, max_V);
/*
